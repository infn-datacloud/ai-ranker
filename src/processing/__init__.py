...
def preprocessing(df: pd.DataFrame, template_complex_types: list) -> pd.DataFrame:
    ...
    df["storage_diff"] = (df["storage_gb_quota"] - df["storage_gb_usage"]) - df[
        "storage_gb_requ"
    ]
    df["instances_diff"] = (df["n_instances_quota"] - df["n_instances_usage"]) - df[
        "n_instances_requ"
    ]
    df["volumes_diff"] = (df["n_volumes_quota"] - df["n_volumes_usage"]) - df[
        "n_volumes_requ"
    ]
    df["floatingips_diff"] = (df["floating_ips_quota"] - df["floating_ips_usage"]) - df[
        "floating_ips_requ"
    ]
    ...
    df["deployment_time"] = np.where(
        df["completed_time"] != 0.0,
        df["completed_time"],
        df["tot_failed_time"] / df["n_failures"],
    )
    ...
    df["failure_percentage"] = df.apply(
        lambda row: calculate_failure_percentage(
            grouped.get_group((row["provider"], row["template_name"])), row
        ),
        axis=1,
    )
    df["avg_success_time"] = df.apply(
        lambda row: calculate_avg_success_time(
            grouped.get_group((row["provider"], row["template_name"])), row
        ),
        axis=1,
    )
    df["avg_failure_time"] = df.apply(
...
def calculate_failure_percentage(group, row):
    mask = (group["timestamp"] <= row["timestamp"]) & (
        group["timestamp"] > row["timestamp"] - pd.Timedelta(days=90)
    )
