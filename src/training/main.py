...
def load_training_data(*, settings: TrainingSettings, logger: Logger) -> pd.DataFrame:
    """Load the training dataset from either a local file or a Kafka topic

    Depends on the provided settings. If `settings.LOCAL_MODE` is True, the dataset is
    loaded from a local file. Otherwise, the dataset is consumed from a Kafka topic
    using the specified settings.

    Args:
        settings (TrainingSettings): Configuration object specifying data source and
            related parameters.
        logger (Logger): Logger instance for logging information and progress.

    Returns:
        pd.DataFrame: The loaded training dataset as a pandas DataFrame.

    Raises:
        ConfigurationError: generated by either `load_local_dataset` or
    ...
    if settings.LOCAL_MODE:
        ...
        consumer = create_kafka_consumer(
            ...
            client_id=settings.KAFKA_TRAINING_CLIENT_NAME,
